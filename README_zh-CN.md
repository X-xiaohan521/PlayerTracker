...
> ❗ **非常推荐**配合**权限管理插件** (例如 `LuckPerms`, [点击这里获取更多信息](https://luckperms.net/wiki/Home)) 使用，可以比默认的“OP”角色拥有更加精确的权限控制。
...
1. #### 基础功能 `/pt track`
    ...
    > 🚀 游戏内玩家可用鼠标 **点击聊天栏坐标快速复制** （注意：我的世界 1.13.x ~ 1.15.x 不支持这个功能）
...
```yml
plugin:
  enabled: true   #控制插件开启/关闭，改为“false”并重载插件可禁用插件
...
- 修复了 MC服务器版本 `1.13.x~1.15.x` 中使用 `/pt track` 指令报错的问题。
