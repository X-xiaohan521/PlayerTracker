...
| 指令            | 功能描述               | 权限       |
|-----------------|-----------------------|-----------|
| `/playertracker track`    | 查询所有在线玩家坐标及状态    | `playertracker.use` |
| `/playertracker track <player>` | 查询指定玩家坐标及状态  | `playertracker.use` |
| `/playertracker log <on/off>` | 开启/关闭日志记录  | `playertracker.admin` |
| `/playertracker viewer add/remove <player>` | 添加/删除对指定玩家的追踪器 | `playertracker.view` |
...
### 🌟 优化
- 优化了命令结构，统一使用 `/playertracker` 前缀
- 可以通过 `/tracker add` 和 `/tracker remove` 来添加或移除追踪器（从而支持：追踪名为 stop 的玩家；同时追踪两个或两个以上玩家）
---
